using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

#pragma warning disable CA1814 // Prefer jagged arrays over multidimensional

namespace Data.Migrations
{
    /// <inheritdoc />
    public partial class nukedcountriesoutofexistance : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropForeignKey(
                name: "FK_Items_Countries_CountryId",
                table: "Items");

            migrationBuilder.DropForeignKey(
                name: "FK_Users_Countries_CountryId",
                table: "Users");

            migrationBuilder.DropTable(
                name: "Countries");

            migrationBuilder.DropIndex(
                name: "IX_Users_CountryId",
                table: "Users");

            migrationBuilder.DropIndex(
                name: "IX_Items_CountryId",
                table: "Items");

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("13b8beec-ab9e-438c-8b44-1a4fbe7f357d"), new Guid("096544e2-3f7a-4c4b-a0f3-3a09d16942d4") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("13b8beec-ab9e-438c-8b44-1a4fbe7f357d"), new Guid("2a6e8599-aace-4515-84cd-2f721aa6bdb4") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("13b8beec-ab9e-438c-8b44-1a4fbe7f357d"), new Guid("db89cae7-a1e6-4791-bc0c-be04e3644acb") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("317f8bcd-f39e-443d-bc03-b12ae4b4a799"), new Guid("2a6e8599-aace-4515-84cd-2f721aa6bdb4") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("317f8bcd-f39e-443d-bc03-b12ae4b4a799"), new Guid("59e3af8b-8698-47c0-b839-393add3b7646") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("317f8bcd-f39e-443d-bc03-b12ae4b4a799"), new Guid("d33ea736-e614-46e2-aabd-6549ef225570") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Guid("096544e2-3f7a-4c4b-a0f3-3a09d16942d4") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Guid("c4497c6a-7833-4021-8e53-0d8f5092ebd6") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Guid("db89cae7-a1e6-4791-bc0c-be04e3644acb") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Guid("f73c580c-5d6e-4b9e-8203-1ec287da619f") });

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("625cbf94-6011-45c7-9a45-a5234490112d"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("04d11fab-4b52-48db-adaa-2a269251b194"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("2c684c00-dc8a-4b5a-b70f-67318817ef09"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("3013509c-d16e-4d6a-90fe-e030de009350"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("39968019-bbfb-4c8d-b5a5-93761a750ad9"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("a020f518-8cb2-49e6-9abc-772eb073400b"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("f3db9f21-fe3f-4c08-8d64-0c7048804506"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("f7bbf3da-049d-4841-b258-d5016f800ffe"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("8d574c63-9b69-4a59-b6fe-7b2048f24029"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("88af4d7c-8ccd-4a3d-9987-7c89886d9d34"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("096544e2-3f7a-4c4b-a0f3-3a09d16942d4"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("2a6e8599-aace-4515-84cd-2f721aa6bdb4"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("59e3af8b-8698-47c0-b839-393add3b7646"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("c4497c6a-7833-4021-8e53-0d8f5092ebd6"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("d33ea736-e614-46e2-aabd-6549ef225570"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("db89cae7-a1e6-4791-bc0c-be04e3644acb"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("f73c580c-5d6e-4b9e-8203-1ec287da619f"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("395b7aa1-9b9c-42e4-9e40-f08335b33459"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("13b8beec-ab9e-438c-8b44-1a4fbe7f357d"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("317f8bcd-f39e-443d-bc03-b12ae4b4a799"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("9300a4e2-a9bc-4995-9fa6-1155c848e40e"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("d0218e1d-c519-49a7-b751-a8f57aabf0cd"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("c6040cb9-65ff-497c-8655-5e85442b9017"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("d9e2a12e-0ee7-45ff-a894-ea06cdd0ae7e"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("e83d458e-1051-4c31-ad16-7333e17d8bb9"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("e9866822-3024-47a4-b6e4-696d65ff229c"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("0f20097b-108d-4383-8f02-5124d84b62da"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("f0f37ca0-e189-44e9-af20-ae067cc60902"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("109961f5-1488-47a1-aeff-cd7f5e89d2b8"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("6123d1cf-6ed9-4516-ae58-f6d247aa6748"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("f7f3aff3-6782-4569-92ea-ee223a6c37ac"));

            migrationBuilder.DropColumn(
                name: "CountryId",
                table: "Users");

            migrationBuilder.DropColumn(
                name: "CountryId",
                table: "Items");

            migrationBuilder.InsertData(
                table: "Categories",
                columns: new[] { "Id", "Description", "Name" },
                values: new object[,]
                {
                    { new Guid("27570ac3-1f57-4dcc-9335-c82bae4cd9a8"), "Everything connected to the field of Economics from microeconomics to macroeconomics", "Economics" },
                    { new Guid("6ac1a39b-eab0-4d83-92e0-e371eeffff3f"), "Everything connected to the field of Chemistry from organic chemistry to inorganic chemistry", "Chemistry" },
                    { new Guid("7a72f249-29bd-41f0-b44f-28391b17b17b"), "Everything connected to the field of Biology from molecular biology to zoology", "Biology" },
                    { new Guid("98d5078f-77f5-4434-a341-ec3962082598"), "Everything connected to the field of informatics and IT", "IT" },
                    { new Guid("a6413346-1556-4533-ad74-00326d536b43"), "Everything connected to the field of Phyisics from quantum mechanics to experimental phyisics", "Phyisics" },
                    { new Guid("b0c97478-5310-4f24-9eee-6049d08f3e97"), "Everything connected to the field of Maths from calculus to applied matheatics", "Maths" }
                });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Claims", "Description", "Email", "LocationName", "Name", "NumberOfEmployees", "Password", "ProfilePicture", "SocialLinks", "Type", "UpdatedAt", "WalletAddress" },
                values: new object[] { new Guid("0168eabd-999e-4c37-9c1a-5f198f2a90c8"), new Dictionary<string, string> { ["trusted_member"] = "true", ["admin"] = "true" }, "Descriptiojn of a fake company", "fakecompany1@gmail.com", "Zagreb", "FakeAdmiinCompany", 10, "/czRqA8NjUMH+Yg6yLePtDg8fMr7GMusRBmc0Rkb77RKYc6d", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 1, new DateTime(2023, 5, 19, 21, 11, 31, 527, DateTimeKind.Utc).AddTicks(1736), "1GSnK58N6XukyGFGbqMDuzt1u3xhGAkTtS" });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Claims", "Description", "Email", "LocationName", "Name", "NumberOfMembers", "Password", "ProfilePicture", "SocialLinks", "Type", "UpdatedAt", "WalletAddress" },
                values: new object[,]
                {
                    { new Guid("56c7cf03-03cd-4964-87a0-c764601a8214"), new Dictionary<string, string> { ["trusted_member"] = "true" }, "Descriptiojn of a fake event organisator", "evets@gmail.com", "Split", "Fake event organisator", 10, "dJ4nwZHPuz071eQdN3yvg9TntSfXVo9E3C7n1Co4uqc6rysd", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 2, new DateTime(2023, 5, 19, 21, 11, 31, 591, DateTimeKind.Utc).AddTicks(3653), "1GSnK5oN6XUkyGFGbqMDuzt1u3xhGAkTtS" },
                    { new Guid("6fb26750-52a5-4fa8-9c93-7baf1fb5424c"), new Dictionary<string, string> { ["trusted_member"] = "true" }, "Descriptiojn of a fake organisation", "organisation@gmail.com", "London", "Organisation", 10, "iDe5eZ0+pu/1twTS4wPdOKTM+fTOIbZmD5NmU1/rRY5ez/18", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 1, new DateTime(2023, 5, 19, 21, 11, 31, 658, DateTimeKind.Utc).AddTicks(8804), "1GsnK58N6XukygFGbqMDuzt1u3xhGAkTtS" }
                });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Claims", "Description", "Email", "LocationName", "Name", "NumberOfEmployees", "Password", "ProfilePicture", "SocialLinks", "Type", "UpdatedAt", "WalletAddress" },
                values: new object[] { new Guid("9a1327a4-99ef-4e7f-9bb2-df6b5afaf47f"), new Dictionary<string, string> { ["trusted_member"] = "true" }, "Descriptiojn of a fake investor", "investor@gmail.com", "London", "Investor", 10, "i0u30/ACXaSNvuDCK/AjSL25F5xj0xrgCW4qeVNarFhFw4B9", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 1, new DateTime(2023, 5, 19, 21, 11, 31, 560, DateTimeKind.Utc).AddTicks(681), "1GSnK58N6XukygFGbqMDuzt1u3xhGAkTtS" });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Claims", "Description", "Email", "LocationName", "Name", "NumberOfMembers", "Password", "ProfilePicture", "SocialLinks", "Type", "UpdatedAt", "WalletAddress" },
                values: new object[] { new Guid("ca252b1a-20cc-420e-a41c-5a458c87a9d9"), new Dictionary<string, string> { ["trusted_member"] = "true", ["admin"] = "true" }, "Descriptiojn of a fake company", "fakecourse@gmail.com", "Hvar", "RandomCourseCompany", 10, "pKzzCTs/gQ7zHXNyc+6wpkMmzk+zuStN2Ub62QbkXhzVDKQ/", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 2, new DateTime(2023, 5, 19, 21, 11, 31, 625, DateTimeKind.Utc).AddTicks(3279), "1GSnK58N6XukyGFGbqMDuzt1u3xhGAkTtS" });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Claims", "Description", "Email", "LocationName", "Name", "NumberOfEmployees", "Password", "ProfilePicture", "SocialLinks", "Type", "UpdatedAt", "WalletAddress" },
                values: new object[] { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Dictionary<string, string> { ["trusted_member"] = "true" }, "Descriptiojn of a fake company", "fakecompany@gmail.com", "Split", "FakeCompany", 10, "ZANw5TTVqnUKiJOZ7tT7YkbtjMBmlIw1u4I5OPY3GLBCghx4", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 1, new DateTime(2023, 5, 19, 21, 11, 31, 496, DateTimeKind.Utc).AddTicks(2674), "1GSnK58N6XUkyGFGbqMDuzt1u3xhGAkTtS" });

            migrationBuilder.InsertData(
                table: "Subcategories",
                columns: new[] { "Id", "CategoryId", "Description", "Name" },
                values: new object[,]
                {
                    { new Guid("1c0a1ce5-dd24-488b-9f33-3e76fcdba76d"), new Guid("27570ac3-1f57-4dcc-9335-c82bae4cd9a8"), "Educational projects related to microeconomics", "Microeconomics" },
                    { new Guid("3b122cd6-8c99-47f0-8519-00cea6942e5e"), new Guid("6ac1a39b-eab0-4d83-92e0-e371eeffff3f"), "Educational projects related to organic chemistry", "Organic chemistry" },
                    { new Guid("48d34f09-27fd-4a8d-ad31-27568ea467e8"), new Guid("98d5078f-77f5-4434-a341-ec3962082598"), "Educational projects related to backend development", "Backend development" },
                    { new Guid("550b4dd1-d494-4923-91a6-7b0a03c2ffb2"), new Guid("27570ac3-1f57-4dcc-9335-c82bae4cd9a8"), "Educational projects related to macroeconomics", "Macroeconomics" },
                    { new Guid("58e896b8-9848-4ccf-9125-017b9a3d5539"), new Guid("98d5078f-77f5-4434-a341-ec3962082598"), "Educational projects related to frontend development", "Frontend development" },
                    { new Guid("5e9e538c-3a18-4ed0-9c8f-b471858fb73a"), new Guid("7a72f249-29bd-41f0-b44f-28391b17b17b"), "Educational projects related to molecular biology", "Molecular biology" },
                    { new Guid("74d049cb-4c0c-4077-8d1f-263eff5bbcb0"), new Guid("b0c97478-5310-4f24-9eee-6049d08f3e97"), "Educational projects related to calculus", "Calculus" },
                    { new Guid("88eac436-3ae8-4746-8790-20078a7bab4f"), new Guid("7a72f249-29bd-41f0-b44f-28391b17b17b"), "Educational projects related to zoology", "Zoology" },
                    { new Guid("8c59ebad-51d8-42a2-88e3-180674691d69"), new Guid("a6413346-1556-4533-ad74-00326d536b43"), "Educational projects related to experimental physics", "Experimental physics" },
                    { new Guid("b047a9d8-4560-4dcd-b460-c87ca7221d91"), new Guid("b0c97478-5310-4f24-9eee-6049d08f3e97"), "Educational projects related to applied mathematics", "Applied mathematics" },
                    { new Guid("d77a7fc9-20e4-435b-af4c-fccdbc486c4f"), new Guid("6ac1a39b-eab0-4d83-92e0-e371eeffff3f"), "Educational projects related to inorganic chemistry", "Inorganic chemistry" },
                    { new Guid("e935e059-ef32-4ed6-8fb3-e39182260540"), new Guid("a6413346-1556-4533-ad74-00326d536b43"), "Educational projects related to quantum mechanics", "Quantum mechanics" }
                });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "Activities", "Capacity", "CategoryId", "Date", "Description", "ExpectedAttendance", "Goal", "Images", "Location", "MainWebsite", "Name", "NotableAttendees", "NotableSpeakers", "OrganisationId", "Prices", "SubcategoryId", "TicketPrice", "Tiers", "Type", "UpdatedAt" },
                values: new object[,]
                {
                    { new Guid("0f236e58-01dd-4102-a46b-d714f9968fd6"), new Dictionary<string, string> { ["12am"] = "pizza", ["1pm"] = "Work" }, 3000, new Guid("6ac1a39b-eab0-4d83-92e0-e371eeffff3f"), new DateTime(2023, 6, 18, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7474), "The greatest design event to exist", 1999, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "Split", "https://randomwebsite.com", "designConference", new List<string> { "CEO" }, new List<string> { "Lead designer" }, new Guid("56c7cf03-03cd-4964-87a0-c764601a8214"), new List<decimal> { 10m, 100m, 1000m }, new Guid("550b4dd1-d494-4923-91a6-7b0a03c2ffb2"), 100m, new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 4, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7480) },
                    { new Guid("2767b9f3-6573-4b88-9df5-a9974601a020"), new Dictionary<string, string> { ["12am"] = "pizza", ["1pm"] = "Work" }, 3000, new Guid("6ac1a39b-eab0-4d83-92e0-e371eeffff3f"), new DateTime(2023, 6, 18, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7458), "The greatest learning event to exist", 1999, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "Zagreb", "https://randomwebsite.com", "LearnConference", new List<string> { "CEO" }, new List<string> { "Lead dev" }, new Guid("56c7cf03-03cd-4964-87a0-c764601a8214"), new List<decimal> { 10m, 100m, 1000m }, new Guid("1c0a1ce5-dd24-488b-9f33-3e76fcdba76d"), 100m, new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 4, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7467) }
                });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "ActiveStudents", "CategoryId", "Curriculum", "Description", "EndDate", "ExpectedApplicants", "ExpectedGraduates", "Goal", "Images", "MainWebsite", "Name", "OrganisationId", "Prices", "StartDate", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[] { new Guid("2bb545e9-a707-4d97-a87c-9a59cacb0b94"), null, new Guid("98d5078f-77f5-4434-a341-ec3962082598"), new Dictionary<string, string> { ["1st lesson"] = "Github", ["2nd lesson"] = "React" }, "The greatest course to exist", new DateTime(2023, 11, 18, 9, 11, 31, 693, DateTimeKind.Utc).AddTicks(7625), 100000, 100, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "https://randomwebsite.com", "React Course", new Guid("56c7cf03-03cd-4964-87a0-c764601a8214"), new List<decimal> { 10m, 100m, 1000m }, new DateTime(2023, 8, 10, 9, 11, 31, 693, DateTimeKind.Utc).AddTicks(7626), new Guid("58e896b8-9848-4ccf-9125-017b9a3d5539"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 0, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7654) });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "AppPurpose", "CategoryId", "Description", "EstimatedNumberOfUsers", "EstimatedRelease", "Features", "Goal", "Images", "MainWebsite", "Markets", "Name", "OrganisationId", "Prices", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[,]
                {
                    { new Guid("4c029b73-6a64-4811-9cfc-127d53a9f70c"), "To teach people how to teach", new Guid("a6413346-1556-4533-ad74-00326d536b43"), "The greatest teaching app to exist", 1000000, new DateTime(2024, 5, 18, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7437), new List<string> { "Good teaching experience", "Amazing customer support" }, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "https://randomwebsite.com", new List<string> { "Android", "Ios" }, "TeachApp", new Guid("ca252b1a-20cc-420e-a41c-5a458c87a9d9"), new List<decimal> { 10m, 100m, 1000m }, new Guid("d77a7fc9-20e4-435b-af4c-fccdbc486c4f"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 1, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7445) },
                    { new Guid("70c53c66-2506-4f04-9fc3-52fa6c974345"), "To teach people how to learn", new Guid("98d5078f-77f5-4434-a341-ec3962082598"), "The greatest learnign app to exist", 1000000, new DateTime(2024, 5, 18, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7401), new List<string> { "Good learinng experience", "Amazing customer support" }, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "https://randomwebsite.com", new List<string> { "Android", "Ios" }, "LearnApp", new Guid("ca252b1a-20cc-420e-a41c-5a458c87a9d9"), new List<decimal> { 10m, 100m, 1000m }, new Guid("58e896b8-9848-4ccf-9125-017b9a3d5539"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 1, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7427) }
                });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "ActiveStudents", "CategoryId", "Curriculum", "Description", "EndDate", "ExpectedApplicants", "ExpectedGraduates", "Goal", "Images", "MainWebsite", "Name", "OrganisationId", "Prices", "StartDate", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[] { new Guid("90dc3649-72c2-4755-8f83-52876342069e"), null, new Guid("b0c97478-5310-4f24-9eee-6049d08f3e97"), new Dictionary<string, string> { ["1st lesson"] = "Github", ["2nd lesson"] = "Webapi" }, "The greatest course to exist", new DateTime(2023, 11, 18, 9, 11, 31, 693, DateTimeKind.Utc).AddTicks(7555), 100000, 100, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "https://randomwebsite.com", "Webapi Course", new Guid("56c7cf03-03cd-4964-87a0-c764601a8214"), new List<decimal> { 10m, 100m, 1000m }, new DateTime(2023, 8, 10, 9, 11, 31, 693, DateTimeKind.Utc).AddTicks(7559), new Guid("b047a9d8-4560-4dcd-b460-c87ca7221d91"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 0, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7569) });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "AvarageDuration", "CategoryId", "Description", "ExpectedAudience", "Goal", "Images", "Lessons", "LessonsDate", "LinksToChannels", "MainWebsite", "Name", "OrganisationId", "Prices", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[,]
                {
                    { new Guid("c31cd899-2cd7-4829-be5f-681186f65240"), 12, new Guid("98d5078f-77f5-4434-a341-ec3962082598"), "The greatest online course to exist", 100, 10000m, new List<string> { "pfp", "stuffaboutapp" }, new Dictionary<string, string> { ["1st lesson"] = "Github", ["2nd lesson"] = "React" }, new List<DateTime> { new DateTime(2023, 5, 21, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7682), new DateTime(2023, 5, 22, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7687), new DateTime(2023, 5, 23, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7687) }, new Dictionary<string, string> { ["youtube"] = "stuff", ["facebook"] = "morestruff" }, "https://randomwebsite.com", "React online Course", new Guid("56c7cf03-03cd-4964-87a0-c764601a8214"), new List<decimal> { 10m, 100m, 1000m }, new Guid("58e896b8-9848-4ccf-9125-017b9a3d5539"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 3, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7740) },
                    { new Guid("df1a1696-3e0a-49e9-8a32-119b2e44943b"), 12, new Guid("98d5078f-77f5-4434-a341-ec3962082598"), "The greatest online course to exist", 100, 10000m, new List<string> { "pfp", "stuffaboutapp" }, new Dictionary<string, string> { ["1st lesson"] = "Github", ["2nd lesson"] = "Wrbapi" }, new List<DateTime> { new DateTime(2023, 5, 21, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7747), new DateTime(2023, 5, 22, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7748), new DateTime(2023, 5, 23, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7749) }, new Dictionary<string, string> { ["youtube"] = "stuff", ["facebook"] = "morestruff" }, "https://randomwebsite.com", "Webapi online Course", new Guid("56c7cf03-03cd-4964-87a0-c764601a8214"), new List<decimal> { 10m, 100m, 1000m }, new Guid("58e896b8-9848-4ccf-9125-017b9a3d5539"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 3, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7771) }
                });

            migrationBuilder.InsertData(
                table: "Investments",
                columns: new[] { "InvestorId", "ItemId", "Amount", "Tier", "UpdatedAt" },
                values: new object[,]
                {
                    { new Guid("0168eabd-999e-4c37-9c1a-5f198f2a90c8"), new Guid("2767b9f3-6573-4b88-9df5-a9974601a020"), 100m, 2, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7778) },
                    { new Guid("0168eabd-999e-4c37-9c1a-5f198f2a90c8"), new Guid("4c029b73-6a64-4811-9cfc-127d53a9f70c"), 30m, 1, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7801) },
                    { new Guid("0168eabd-999e-4c37-9c1a-5f198f2a90c8"), new Guid("c31cd899-2cd7-4829-be5f-681186f65240"), 10000m, 3, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7780) },
                    { new Guid("9a1327a4-99ef-4e7f-9bb2-df6b5afaf47f"), new Guid("0f236e58-01dd-4102-a46b-d714f9968fd6"), 300m, 2, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7806) },
                    { new Guid("9a1327a4-99ef-4e7f-9bb2-df6b5afaf47f"), new Guid("4c029b73-6a64-4811-9cfc-127d53a9f70c"), 20m, 1, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7785) },
                    { new Guid("9a1327a4-99ef-4e7f-9bb2-df6b5afaf47f"), new Guid("70c53c66-2506-4f04-9fc3-52fa6c974345"), 40m, 1, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7809) },
                    { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Guid("0f236e58-01dd-4102-a46b-d714f9968fd6"), 200m, 2, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7787) },
                    { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Guid("2bb545e9-a707-4d97-a87c-9a59cacb0b94"), 30000m, 3, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7807) },
                    { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Guid("70c53c66-2506-4f04-9fc3-52fa6c974345"), 10m, 1, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7773) },
                    { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Guid("df1a1696-3e0a-49e9-8a32-119b2e44943b"), 20000m, 3, new DateTime(2023, 5, 19, 21, 11, 31, 693, DateTimeKind.Utc).AddTicks(7799) }
                });
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("0168eabd-999e-4c37-9c1a-5f198f2a90c8"), new Guid("2767b9f3-6573-4b88-9df5-a9974601a020") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("0168eabd-999e-4c37-9c1a-5f198f2a90c8"), new Guid("4c029b73-6a64-4811-9cfc-127d53a9f70c") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("0168eabd-999e-4c37-9c1a-5f198f2a90c8"), new Guid("c31cd899-2cd7-4829-be5f-681186f65240") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("9a1327a4-99ef-4e7f-9bb2-df6b5afaf47f"), new Guid("0f236e58-01dd-4102-a46b-d714f9968fd6") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("9a1327a4-99ef-4e7f-9bb2-df6b5afaf47f"), new Guid("4c029b73-6a64-4811-9cfc-127d53a9f70c") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("9a1327a4-99ef-4e7f-9bb2-df6b5afaf47f"), new Guid("70c53c66-2506-4f04-9fc3-52fa6c974345") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Guid("0f236e58-01dd-4102-a46b-d714f9968fd6") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Guid("2bb545e9-a707-4d97-a87c-9a59cacb0b94") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Guid("70c53c66-2506-4f04-9fc3-52fa6c974345") });

            migrationBuilder.DeleteData(
                table: "Investments",
                keyColumns: new[] { "InvestorId", "ItemId" },
                keyValues: new object[] { new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"), new Guid("df1a1696-3e0a-49e9-8a32-119b2e44943b") });

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("90dc3649-72c2-4755-8f83-52876342069e"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("3b122cd6-8c99-47f0-8519-00cea6942e5e"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("48d34f09-27fd-4a8d-ad31-27568ea467e8"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("5e9e538c-3a18-4ed0-9c8f-b471858fb73a"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("74d049cb-4c0c-4077-8d1f-263eff5bbcb0"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("88eac436-3ae8-4746-8790-20078a7bab4f"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("8c59ebad-51d8-42a2-88e3-180674691d69"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("e935e059-ef32-4ed6-8fb3-e39182260540"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("6fb26750-52a5-4fa8-9c93-7baf1fb5424c"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("7a72f249-29bd-41f0-b44f-28391b17b17b"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("0f236e58-01dd-4102-a46b-d714f9968fd6"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("2767b9f3-6573-4b88-9df5-a9974601a020"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("2bb545e9-a707-4d97-a87c-9a59cacb0b94"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("4c029b73-6a64-4811-9cfc-127d53a9f70c"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("70c53c66-2506-4f04-9fc3-52fa6c974345"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("c31cd899-2cd7-4829-be5f-681186f65240"));

            migrationBuilder.DeleteData(
                table: "Items",
                keyColumn: "Id",
                keyValue: new Guid("df1a1696-3e0a-49e9-8a32-119b2e44943b"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("b047a9d8-4560-4dcd-b460-c87ca7221d91"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("0168eabd-999e-4c37-9c1a-5f198f2a90c8"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("9a1327a4-99ef-4e7f-9bb2-df6b5afaf47f"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("e8b0c149-6eb0-4c7c-a6ce-0bc611d6811c"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("a6413346-1556-4533-ad74-00326d536b43"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("b0c97478-5310-4f24-9eee-6049d08f3e97"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("1c0a1ce5-dd24-488b-9f33-3e76fcdba76d"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("550b4dd1-d494-4923-91a6-7b0a03c2ffb2"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("58e896b8-9848-4ccf-9125-017b9a3d5539"));

            migrationBuilder.DeleteData(
                table: "Subcategories",
                keyColumn: "Id",
                keyValue: new Guid("d77a7fc9-20e4-435b-af4c-fccdbc486c4f"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("56c7cf03-03cd-4964-87a0-c764601a8214"));

            migrationBuilder.DeleteData(
                table: "Users",
                keyColumn: "Id",
                keyValue: new Guid("ca252b1a-20cc-420e-a41c-5a458c87a9d9"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("27570ac3-1f57-4dcc-9335-c82bae4cd9a8"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("6ac1a39b-eab0-4d83-92e0-e371eeffff3f"));

            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: new Guid("98d5078f-77f5-4434-a341-ec3962082598"));

            migrationBuilder.AddColumn<Guid>(
                name: "CountryId",
                table: "Users",
                type: "uuid",
                nullable: false,
                defaultValue: new Guid("00000000-0000-0000-0000-000000000000"));

            migrationBuilder.AddColumn<Guid>(
                name: "CountryId",
                table: "Items",
                type: "uuid",
                nullable: false,
                defaultValue: new Guid("00000000-0000-0000-0000-000000000000"));

            migrationBuilder.CreateTable(
                name: "Countries",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uuid", nullable: false),
                    Flag = table.Column<string>(type: "text", nullable: false),
                    Name = table.Column<string>(type: "text", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_Countries", x => x.Id);
                });

            migrationBuilder.InsertData(
                table: "Categories",
                columns: new[] { "Id", "Description", "Name" },
                values: new object[,]
                {
                    { new Guid("109961f5-1488-47a1-aeff-cd7f5e89d2b8"), "Everything connected to the field of informatics and IT", "IT" },
                    { new Guid("6123d1cf-6ed9-4516-ae58-f6d247aa6748"), "Everything connected to the field of Chemistry from organic chemistry to inorganic chemistry", "Chemistry" },
                    { new Guid("88af4d7c-8ccd-4a3d-9987-7c89886d9d34"), "Everything connected to the field of Biology from molecular biology to zoology", "Biology" },
                    { new Guid("9300a4e2-a9bc-4995-9fa6-1155c848e40e"), "Everything connected to the field of Maths from calculus to applied matheatics", "Maths" },
                    { new Guid("d0218e1d-c519-49a7-b751-a8f57aabf0cd"), "Everything connected to the field of Phyisics from quantum mechanics to experimental phyisics", "Phyisics" },
                    { new Guid("f7f3aff3-6782-4569-92ea-ee223a6c37ac"), "Everything connected to the field of Economics from microeconomics to macroeconomics", "Economics" }
                });

            migrationBuilder.InsertData(
                table: "Countries",
                columns: new[] { "Id", "Flag", "Name" },
                values: new object[,]
                {
                    { new Guid("00ffbe82-50c9-4dca-9a5c-9b15f7787b6e"), "data:image/png;base64,", "France" },
                    { new Guid("018ad2a0-336c-4003-8381-09b2334cf629"), "data:image/png;base64,", "Philippines" },
                    { new Guid("02cd29a3-d442-4e6a-8f11-05c131dab331"), "data:image/png;base64,", "France" },
                    { new Guid("0412104d-1f17-4058-8b48-07c918db14b1"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("0a571dca-8d2a-47cf-88c6-28047e43d6d2"), "data:image/png;base64,", "France" },
                    { new Guid("1100edd5-ba3b-460b-a29d-11037297dcbd"), "data:image/png;base64,", "Iran" },
                    { new Guid("198029ee-71a2-45c2-9531-2845dafb5721"), "data:image/png;base64,", "Cameroon" },
                    { new Guid("2015409a-9fd3-47e9-a890-121a51fb68a8"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("24dbb087-8c4e-4203-af53-754b20424359"), "data:image/png;base64,", "Morocco" },
                    { new Guid("272fd043-252b-465b-bbc5-0437308ab85a"), "data:image/png;base64,", "China" },
                    { new Guid("2befdb27-bf1e-44fb-b1d8-9c44b5ac50cf"), "data:image/png;base64,", "Russia" },
                    { new Guid("2cd0a77d-ed1d-4815-8864-cbbd8c3eb5c3"), "data:image/png;base64,", "Portugal" },
                    { new Guid("2d188a07-93bc-49d6-83a3-cf6b019769c6"), "data:image/png;base64,", "Sweden" },
                    { new Guid("311a4729-4561-42c4-9964-834628241e9e"), "data:image/png;base64,", "Brazil" },
                    { new Guid("32a5e9fb-3678-4b29-8bf0-79a29a21779b"), "data:image/png;base64,", "Czech Republic" },
                    { new Guid("33a58e20-1d16-44ae-aa76-cf4f5b31988e"), "data:image/png;base64,", "Czech Republic" },
                    { new Guid("37b310c8-f129-484e-a74c-48e56a893cd2"), "data:image/png;base64,", "Panama" },
                    { new Guid("38e45300-72cc-4373-aa2b-8ded49ff91dd"), "data:image/png;base64,", "France" },
                    { new Guid("399853f2-76ab-4730-8949-8bc059607f55"), "data:image/png;base64,", "Poland" },
                    { new Guid("3b7ae228-ad6a-430e-b27a-70f1aae62d8b"), "data:image/png;base64,", "Poland" },
                    { new Guid("3f1717f6-5fe1-420b-9851-56379fc491a3"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("3fa50ac3-f8ad-4e0e-8c18-b09a2afd9e76"), "data:image/png;base64,", "Mexico" },
                    { new Guid("403c5ee9-037c-4baf-9c65-1d976b513b1b"), "data:image/png;base64,", "Portugal" },
                    { new Guid("43228272-d7e2-4c8d-94e6-ffdbc497dd26"), "data:image/png;base64,", "China" },
                    { new Guid("46d35856-496c-44cb-91ed-fb6dc5ca5bc6"), "data:image/png;base64,", "Philippines" },
                    { new Guid("4b586d5b-3d0d-43b5-9b7d-1ab716ac3c47"), "data:image/png;base64,", "China" },
                    { new Guid("4e7d8caa-e50d-45bf-a164-cb2c7a2d306c"), "data:image/png;base64,", "Russia" },
                    { new Guid("4efa9713-a4c3-41ca-894c-d992f38f0010"), "data:image/png;base64,", "United States" },
                    { new Guid("4f023929-1fd8-44cc-a01a-19b8b1c8f59f"), "data:image/png;base64,", "Ukraine" },
                    { new Guid("54541d45-5bba-406b-b3c8-3245fa3f340f"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("58d5ece9-8d79-4cdb-9841-d8c52e197705"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("59ef4fa7-c66f-4756-9292-802afb921896"), "data:image/png;base64,", "Poland" },
                    { new Guid("5a05388f-b1d6-409e-8649-afbaa7ff48a5"), "data:image/png;base64,", "Thailand" },
                    { new Guid("5a47f642-4382-43b8-8967-763a6771b1d4"), "data:image/png;base64,", "China" },
                    { new Guid("5f41c37e-86e6-4897-8db0-897bc2001ddb"), "data:image/png;base64,", "Costa Rica" },
                    { new Guid("642697fc-9261-4778-9d5e-124c479b3c5a"), "data:image/png;base64,", "Ecuador" },
                    { new Guid("6bcf4ee5-2bd9-4e5d-b7ae-fb3ec850a6e5"), "data:image/png;base64,", "Sweden" },
                    { new Guid("6ce33558-c930-40f2-9a5e-5e133aeca71a"), "data:image/png;base64,", "Ukraine" },
                    { new Guid("6def8637-59f5-4b18-944e-a247ead1a40f"), "data:image/png;base64,", "Russia" },
                    { new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "data:image/png;base64,", "Russia" },
                    { new Guid("739e24d9-c8b1-45ea-a103-7891506f1bea"), "data:image/png;base64,", "Japan" },
                    { new Guid("74a72e22-04be-45ac-bfec-168967029eac"), "data:image/png;base64,", "Jamaica" },
                    { new Guid("7503e1bc-ec4a-459d-b60d-ce970d9c4413"), "data:image/png;base64,", "China" },
                    { new Guid("7615b77f-8bfd-4573-b74b-91d5e953b839"), "data:image/png;base64,", "China" },
                    { new Guid("773b74e2-65f3-4894-bc05-66d30e95c7d4"), "data:image/png;base64,", "China" },
                    { new Guid("779f7e60-b11f-46c2-bb35-bcfb18b6281d"), "data:image/png;base64,", "Honduras" },
                    { new Guid("784732bf-5a1d-4235-ad8e-abe781750788"), "data:image/png;base64,", "Cuba" },
                    { new Guid("79bb4f3e-2be7-4cf0-9288-f6fdff7fc06f"), "data:image/png;base64,", "China" },
                    { new Guid("7c6febe9-f885-4e76-9659-2ab735cf8c31"), "data:image/png;base64,", "France" },
                    { new Guid("7cc9b334-2826-4455-8eae-aaaf1081017e"), "data:image/png;base64,", "Belarus" },
                    { new Guid("80516c4c-d73c-4b28-b6a7-40528ca5f9dc"), "data:image/png;base64,", "Ukraine" },
                    { new Guid("811ab45f-b390-4993-a73f-190ca020d5a0"), "data:image/png;base64,", "China" },
                    { new Guid("815ffb39-2c16-4ce8-871a-4c0b6a869a3b"), "data:image/png;base64,", "Finland" },
                    { new Guid("83230c7a-59cf-4051-8a33-fa6162440f08"), "data:image/png;base64,", "Latvia" },
                    { new Guid("871e10e6-e377-4277-ad3e-a5050a5eb3ad"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("877f135b-3388-48c0-8dd6-3a52bd8d09ed"), "data:image/png;base64,", "Poland" },
                    { new Guid("8b2c17db-39d1-474a-94d9-9a16d391f69d"), "data:image/png;base64,", "China" },
                    { new Guid("8de8644a-881a-4116-9d5b-79e85a2522ec"), "data:image/png;base64,", "Sri Lanka" },
                    { new Guid("8e0d47bb-1110-4516-82d5-132decbffedd"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("8e3b1114-3ddc-45c6-9e10-b3b5673673f4"), "data:image/png;base64,", "Russia" },
                    { new Guid("9263d281-bac2-4b26-ad2f-ce2ee9a94948"), "data:image/png;base64,", "Dominican Republic" },
                    { new Guid("926aa29b-c8c1-4b30-bc7a-fb8bb6b8987a"), "data:image/png;base64,", "Guatemala" },
                    { new Guid("953dcc64-b013-4590-9a8d-9d5e46040bd2"), "data:image/png;base64,", "Ecuador" },
                    { new Guid("95a21465-b660-45f8-bb69-3acbc623512d"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("9cd72b99-f827-4525-b7b4-de4cdd749f5b"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("a7f06892-ced7-4106-a489-ee31913cd043"), "data:image/png;base64,", "France" },
                    { new Guid("aae50d42-4d76-40d5-a5e0-f111849757e9"), "data:image/png;base64,", "France" },
                    { new Guid("ab606cc0-6c64-46b1-8781-b01d7f1f8996"), "data:image/png;base64,", "Greece" },
                    { new Guid("b266491e-0a84-432a-92f3-1ea2d44f8b57"), "data:image/png;base64,", "Philippines" },
                    { new Guid("b72a2748-887e-4008-9742-ace66330f125"), "data:image/png;base64,", "Haiti" },
                    { new Guid("b736d3e3-ee09-4501-b293-6b7bfd9eb5b6"), "data:image/png;base64,", "Russia" },
                    { new Guid("b747e046-3e03-4911-903e-a7fd8b6a7386"), "data:image/png;base64,", "Iraq" },
                    { new Guid("baf8619a-1691-4bda-97ab-1734f0528582"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("c12f5898-1ec0-40f7-b257-989efe8bf2f1"), "data:image/png;base64,", "China" },
                    { new Guid("c142532a-0f64-4fdd-880e-bd643427bf4e"), "data:image/png;base64,", "China" },
                    { new Guid("c1c16be7-db86-450c-92d8-c771ba2f9007"), "data:image/png;base64,", "Malaysia" },
                    { new Guid("c3aa6a71-5c00-476d-9df3-459b5c911a06"), "data:image/png;base64,", "Brazil" },
                    { new Guid("cccdc66a-9c99-4579-ac68-96b9cfb5f141"), "data:image/png;base64,", "China" },
                    { new Guid("ccd90af0-c30f-4c55-be53-88ca82156646"), "data:image/png;base64,", "Poland" },
                    { new Guid("ce207ea0-63ca-4c95-ae66-d03da416623a"), "data:image/png;base64,", "China" },
                    { new Guid("ceb8bae3-7591-4230-86d8-263ec54fb64b"), "data:image/png;base64,", "Japan" },
                    { new Guid("d14ac6dc-d1c6-4153-a270-d0cdb40d26a8"), "data:image/png;base64,", "Argentina" },
                    { new Guid("d1dab172-b163-43c4-8682-1f65fb6c4b4e"), "data:image/png;base64,", "U.S. Virgin Islands" },
                    { new Guid("d4e7c904-741a-4b8c-9c62-177c00a00e60"), "data:image/png;base64,", "China" },
                    { new Guid("d8a0392d-58ef-4054-816a-d0c9c8991592"), "data:image/png;base64,", "China" },
                    { new Guid("decf433e-656a-4687-9133-e7290b55d4bb"), "data:image/png;base64,", "Russia" },
                    { new Guid("def3e98c-1fdc-4140-817e-8c549e387571"), "data:image/png;base64,", "Philippines" },
                    { new Guid("e0cde9cd-ccea-4865-87c5-67864a805e66"), "data:image/png;base64,", "Honduras" },
                    { new Guid("e1a31461-44d7-4328-827f-13bf7b101809"), "data:image/png;base64,", "Albania" },
                    { new Guid("e5df787b-1cc1-432c-8ddc-870df03546c0"), "data:image/png;base64,", "Indonesia" },
                    { new Guid("eb3ad19b-71a4-4b43-82b4-92ecfb0249f4"), "data:image/png;base64,", "Ireland" },
                    { new Guid("eddf1841-22c6-4a05-8d49-12bbfcaf8cc5"), "data:image/png;base64,", "Mexico" },
                    { new Guid("ee0abf82-910b-466d-a26b-faa0d0dea9a6"), "data:image/png;base64,", "Netherlands" },
                    { new Guid("f0a0f73c-fb4c-4003-8c56-227ea719d065"), "data:image/png;base64,", "Croatia" },
                    { new Guid("f0cf1c9a-2f89-4437-9640-6f94770d9ede"), "data:image/png;base64,", "Azerbaijan" },
                    { new Guid("f336648e-787f-486d-95c4-089bb01616af"), "data:image/png;base64,", "Poland" },
                    { new Guid("f5e60a68-6ac3-4a22-b9ff-26e594a991e4"), "data:image/png;base64,", "China" },
                    { new Guid("f728a00d-0e91-4594-8ad6-85cd868360ef"), "data:image/png;base64,", "Portugal" },
                    { new Guid("fbd006e7-b7fb-488b-bf54-5b01f1eb5fd6"), "data:image/png;base64,", "China" },
                    { new Guid("fbffc3f9-14c9-4782-bfd9-56eef7e4cdec"), "data:image/png;base64,", "Syria" }
                });

            migrationBuilder.InsertData(
                table: "Subcategories",
                columns: new[] { "Id", "CategoryId", "Description", "Name" },
                values: new object[,]
                {
                    { new Guid("04d11fab-4b52-48db-adaa-2a269251b194"), new Guid("6123d1cf-6ed9-4516-ae58-f6d247aa6748"), "Educational projects related to organic chemistry", "Organic chemistry" },
                    { new Guid("2c684c00-dc8a-4b5a-b70f-67318817ef09"), new Guid("9300a4e2-a9bc-4995-9fa6-1155c848e40e"), "Educational projects related to calculus", "Calculus" },
                    { new Guid("3013509c-d16e-4d6a-90fe-e030de009350"), new Guid("d0218e1d-c519-49a7-b751-a8f57aabf0cd"), "Educational projects related to quantum mechanics", "Quantum mechanics" },
                    { new Guid("395b7aa1-9b9c-42e4-9e40-f08335b33459"), new Guid("9300a4e2-a9bc-4995-9fa6-1155c848e40e"), "Educational projects related to applied mathematics", "Applied mathematics" },
                    { new Guid("39968019-bbfb-4c8d-b5a5-93761a750ad9"), new Guid("88af4d7c-8ccd-4a3d-9987-7c89886d9d34"), "Educational projects related to molecular biology", "Molecular biology" },
                    { new Guid("a020f518-8cb2-49e6-9abc-772eb073400b"), new Guid("109961f5-1488-47a1-aeff-cd7f5e89d2b8"), "Educational projects related to backend development", "Backend development" },
                    { new Guid("c6040cb9-65ff-497c-8655-5e85442b9017"), new Guid("6123d1cf-6ed9-4516-ae58-f6d247aa6748"), "Educational projects related to inorganic chemistry", "Inorganic chemistry" },
                    { new Guid("d9e2a12e-0ee7-45ff-a894-ea06cdd0ae7e"), new Guid("109961f5-1488-47a1-aeff-cd7f5e89d2b8"), "Educational projects related to frontend development", "Frontend development" },
                    { new Guid("e83d458e-1051-4c31-ad16-7333e17d8bb9"), new Guid("f7f3aff3-6782-4569-92ea-ee223a6c37ac"), "Educational projects related to microeconomics", "Microeconomics" },
                    { new Guid("e9866822-3024-47a4-b6e4-696d65ff229c"), new Guid("f7f3aff3-6782-4569-92ea-ee223a6c37ac"), "Educational projects related to macroeconomics", "Macroeconomics" },
                    { new Guid("f3db9f21-fe3f-4c08-8d64-0c7048804506"), new Guid("d0218e1d-c519-49a7-b751-a8f57aabf0cd"), "Educational projects related to experimental physics", "Experimental physics" },
                    { new Guid("f7bbf3da-049d-4841-b258-d5016f800ffe"), new Guid("88af4d7c-8ccd-4a3d-9987-7c89886d9d34"), "Educational projects related to zoology", "Zoology" }
                });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Claims", "CountryId", "Description", "Email", "LocationName", "Name", "NumberOfMembers", "Password", "ProfilePicture", "SocialLinks", "Type", "UpdatedAt", "WalletAddress" },
                values: new object[] { new Guid("0f20097b-108d-4383-8f02-5124d84b62da"), new Dictionary<string, string> { ["trusted_member"] = "true" }, new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "Descriptiojn of a fake event organisator", "evets@gmail.com", "Split", "Fake event organisator", 10, "LJ6Oduii8SOLksTbw4WwkJLEp5Y/J84inlNccP82WUbEnHVk", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 2, new DateTime(2023, 5, 18, 11, 49, 23, 425, DateTimeKind.Utc).AddTicks(5883), "1GSnK5oN6XUkyGFGbqMDuzt1u3xhGAkTtS" });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Claims", "CountryId", "Description", "Email", "LocationName", "Name", "NumberOfEmployees", "Password", "ProfilePicture", "SocialLinks", "Type", "UpdatedAt", "WalletAddress" },
                values: new object[,]
                {
                    { new Guid("13b8beec-ab9e-438c-8b44-1a4fbe7f357d"), new Dictionary<string, string> { ["trusted_member"] = "true" }, new Guid("def3e98c-1fdc-4140-817e-8c549e387571"), "Descriptiojn of a fake investor", "investor@gmail.com", "London", "Investor", 10, "HAo7dvw0nwJ/Xfra2coDi6nOduOl+z9X0OtGlE0YiHhi6g1a", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 1, new DateTime(2023, 5, 18, 11, 49, 23, 396, DateTimeKind.Utc).AddTicks(415), "1GSnK58N6XukygFGbqMDuzt1u3xhGAkTtS" },
                    { new Guid("317f8bcd-f39e-443d-bc03-b12ae4b4a799"), new Dictionary<string, string> { ["trusted_member"] = "true", ["admin"] = "true" }, new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "Descriptiojn of a fake company", "fakecompany1@gmail.com", "Zagreb", "FakeAdmiinCompany", 10, "LUWW4rLlQ+56XKNgC6x9NKrOT8TY6Qesu3DN1q/S9jIp34kZ", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 1, new DateTime(2023, 5, 18, 11, 49, 23, 366, DateTimeKind.Utc).AddTicks(429), "1GSnK58N6XukyGFGbqMDuzt1u3xhGAkTtS" },
                    { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Dictionary<string, string> { ["trusted_member"] = "true" }, new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "Descriptiojn of a fake company", "fakecompany@gmail.com", "Split", "FakeCompany", 10, "FEdZ6Bcyu2jIYBBPO+K1+erX95H5MQjDJqtHlPXuie76Q05j", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 1, new DateTime(2023, 5, 18, 11, 49, 23, 334, DateTimeKind.Utc).AddTicks(5030), "1GSnK58N6XUkyGFGbqMDuzt1u3xhGAkTtS" }
                });

            migrationBuilder.InsertData(
                table: "Users",
                columns: new[] { "Id", "Claims", "CountryId", "Description", "Email", "LocationName", "Name", "NumberOfMembers", "Password", "ProfilePicture", "SocialLinks", "Type", "UpdatedAt", "WalletAddress" },
                values: new object[,]
                {
                    { new Guid("8d574c63-9b69-4a59-b6fe-7b2048f24029"), new Dictionary<string, string> { ["trusted_member"] = "true" }, new Guid("def3e98c-1fdc-4140-817e-8c549e387571"), "Descriptiojn of a fake organisation", "organisation@gmail.com", "London", "Organisation", 10, "QEoc2noedCWdb0bALtF/APdb3XI9wKiKhvqVcCJNKNCH+okt", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 1, new DateTime(2023, 5, 18, 11, 49, 23, 484, DateTimeKind.Utc).AddTicks(6859), "1GsnK58N6XukygFGbqMDuzt1u3xhGAkTtS" },
                    { new Guid("f0f37ca0-e189-44e9-af20-ae067cc60902"), new Dictionary<string, string> { ["trusted_member"] = "true", ["admin"] = "true" }, new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "Descriptiojn of a fake company", "fakecourse@gmail.com", "Hvar", "RandomCourseCompany", 10, "hNG2/GsJ9oKbRen07WS1gTVPFjFp7g7iQ/Ywe/gbhcqm6M5V", "pfp", new Dictionary<string, string> { ["facebook"] = "facebook.com" }, 2, new DateTime(2023, 5, 18, 11, 49, 23, 455, DateTimeKind.Utc).AddTicks(1840), "1GSnK58N6XukyGFGbqMDuzt1u3xhGAkTtS" }
                });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "AppPurpose", "CategoryId", "CountryId", "Description", "EstimatedNumberOfUsers", "EstimatedRelease", "Features", "Goal", "Images", "MainWebsite", "Markets", "Name", "OrganisationId", "Prices", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[,]
                {
                    { new Guid("096544e2-3f7a-4c4b-a0f3-3a09d16942d4"), "To teach people how to learn", new Guid("109961f5-1488-47a1-aeff-cd7f5e89d2b8"), new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "The greatest learnign app to exist", 1000000, new DateTime(2024, 5, 17, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3044), new List<string> { "Good learinng experience", "Amazing customer support" }, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "https://randomwebsite.com", new List<string> { "Android", "Ios" }, "LearnApp", new Guid("f0f37ca0-e189-44e9-af20-ae067cc60902"), new List<decimal> { 10m, 100m, 1000m }, new Guid("d9e2a12e-0ee7-45ff-a894-ea06cdd0ae7e"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 1, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3073) },
                    { new Guid("2a6e8599-aace-4515-84cd-2f721aa6bdb4"), "To teach people how to teach", new Guid("d0218e1d-c519-49a7-b751-a8f57aabf0cd"), new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "The greatest teaching app to exist", 1000000, new DateTime(2024, 5, 17, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3079), new List<string> { "Good teaching experience", "Amazing customer support" }, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "https://randomwebsite.com", new List<string> { "Android", "Ios" }, "TeachApp", new Guid("f0f37ca0-e189-44e9-af20-ae067cc60902"), new List<decimal> { 10m, 100m, 1000m }, new Guid("c6040cb9-65ff-497c-8655-5e85442b9017"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 1, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3087) }
                });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "AvarageDuration", "CategoryId", "CountryId", "Description", "ExpectedAudience", "Goal", "Images", "Lessons", "LessonsDate", "LinksToChannels", "MainWebsite", "Name", "OrganisationId", "Prices", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[] { new Guid("59e3af8b-8698-47c0-b839-393add3b7646"), 12, new Guid("109961f5-1488-47a1-aeff-cd7f5e89d2b8"), new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "The greatest online course to exist", 100, 10000m, new List<string> { "pfp", "stuffaboutapp" }, new Dictionary<string, string> { ["1st lesson"] = "Github", ["2nd lesson"] = "React" }, new List<DateTime> { new DateTime(2023, 5, 20, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3301), new DateTime(2023, 5, 21, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3305), new DateTime(2023, 5, 22, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3305) }, new Dictionary<string, string> { ["youtube"] = "stuff", ["facebook"] = "morestruff" }, "https://randomwebsite.com", "React online Course", new Guid("0f20097b-108d-4383-8f02-5124d84b62da"), new List<decimal> { 10m, 100m, 1000m }, new Guid("d9e2a12e-0ee7-45ff-a894-ea06cdd0ae7e"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 3, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3345) });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "ActiveStudents", "CategoryId", "CountryId", "Curriculum", "Description", "EndDate", "ExpectedApplicants", "ExpectedGraduates", "Goal", "Images", "MainWebsite", "Name", "OrganisationId", "Prices", "StartDate", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[] { new Guid("625cbf94-6011-45c7-9a45-a5234490112d"), null, new Guid("9300a4e2-a9bc-4995-9fa6-1155c848e40e"), new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), new Dictionary<string, string> { ["1st lesson"] = "Github", ["2nd lesson"] = "Webapi" }, "The greatest course to exist", new DateTime(2023, 11, 16, 23, 49, 23, 516, DateTimeKind.Utc).AddTicks(3209), 100000, 100, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "https://randomwebsite.com", "Webapi Course", new Guid("0f20097b-108d-4383-8f02-5124d84b62da"), new List<decimal> { 10m, 100m, 1000m }, new DateTime(2023, 8, 8, 23, 49, 23, 516, DateTimeKind.Utc).AddTicks(3215), new Guid("395b7aa1-9b9c-42e4-9e40-f08335b33459"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 0, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3224) });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "AvarageDuration", "CategoryId", "CountryId", "Description", "ExpectedAudience", "Goal", "Images", "Lessons", "LessonsDate", "LinksToChannels", "MainWebsite", "Name", "OrganisationId", "Prices", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[] { new Guid("c4497c6a-7833-4021-8e53-0d8f5092ebd6"), 12, new Guid("109961f5-1488-47a1-aeff-cd7f5e89d2b8"), new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), "The greatest online course to exist", 100, 10000m, new List<string> { "pfp", "stuffaboutapp" }, new Dictionary<string, string> { ["1st lesson"] = "Github", ["2nd lesson"] = "Wrbapi" }, new List<DateTime> { new DateTime(2023, 5, 20, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3354), new DateTime(2023, 5, 21, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3355), new DateTime(2023, 5, 22, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3356) }, new Dictionary<string, string> { ["youtube"] = "stuff", ["facebook"] = "morestruff" }, "https://randomwebsite.com", "Webapi online Course", new Guid("0f20097b-108d-4383-8f02-5124d84b62da"), new List<decimal> { 10m, 100m, 1000m }, new Guid("d9e2a12e-0ee7-45ff-a894-ea06cdd0ae7e"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 3, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3373) });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "Activities", "Capacity", "CategoryId", "CountryId", "Date", "Description", "ExpectedAttendance", "Goal", "Images", "Location", "MainWebsite", "Name", "NotableAttendees", "NotableSpeakers", "OrganisationId", "Prices", "SubcategoryId", "TicketPrice", "Tiers", "Type", "UpdatedAt" },
                values: new object[,]
                {
                    { new Guid("d33ea736-e614-46e2-aabd-6549ef225570"), new Dictionary<string, string> { ["12am"] = "pizza", ["1pm"] = "Work" }, 3000, new Guid("6123d1cf-6ed9-4516-ae58-f6d247aa6748"), new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), new DateTime(2023, 6, 17, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3106), "The greatest learning event to exist", 1999, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "Zagreb", "https://randomwebsite.com", "LearnConference", new List<string> { "CEO" }, new List<string> { "Lead dev" }, new Guid("0f20097b-108d-4383-8f02-5124d84b62da"), new List<decimal> { 10m, 100m, 1000m }, new Guid("e83d458e-1051-4c31-ad16-7333e17d8bb9"), 100m, new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 4, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3113) },
                    { new Guid("db89cae7-a1e6-4791-bc0c-be04e3644acb"), new Dictionary<string, string> { ["12am"] = "pizza", ["1pm"] = "Work" }, 3000, new Guid("6123d1cf-6ed9-4516-ae58-f6d247aa6748"), new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), new DateTime(2023, 6, 17, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3120), "The greatest design event to exist", 1999, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "Split", "https://randomwebsite.com", "designConference", new List<string> { "CEO" }, new List<string> { "Lead designer" }, new Guid("0f20097b-108d-4383-8f02-5124d84b62da"), new List<decimal> { 10m, 100m, 1000m }, new Guid("e9866822-3024-47a4-b6e4-696d65ff229c"), 100m, new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 4, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3138) }
                });

            migrationBuilder.InsertData(
                table: "Items",
                columns: new[] { "Id", "ActiveStudents", "CategoryId", "CountryId", "Curriculum", "Description", "EndDate", "ExpectedApplicants", "ExpectedGraduates", "Goal", "Images", "MainWebsite", "Name", "OrganisationId", "Prices", "StartDate", "SubcategoryId", "Tiers", "Type", "UpdatedAt" },
                values: new object[] { new Guid("f73c580c-5d6e-4b9e-8203-1ec287da619f"), null, new Guid("109961f5-1488-47a1-aeff-cd7f5e89d2b8"), new Guid("729fa549-7a78-4621-aeb5-7314f12f6803"), new Dictionary<string, string> { ["1st lesson"] = "Github", ["2nd lesson"] = "React" }, "The greatest course to exist", new DateTime(2023, 11, 16, 23, 49, 23, 516, DateTimeKind.Utc).AddTicks(3231), 100000, 100, 10000m, new List<string> { "pfp", "stuffaboutapp" }, "https://randomwebsite.com", "React Course", new Guid("0f20097b-108d-4383-8f02-5124d84b62da"), new List<decimal> { 10m, 100m, 1000m }, new DateTime(2023, 8, 8, 23, 49, 23, 516, DateTimeKind.Utc).AddTicks(3232), new Guid("d9e2a12e-0ee7-45ff-a894-ea06cdd0ae7e"), new Dictionary<string, string> { ["Bronze"] = "Basic tier", ["Silver"] = "Middle tier", ["Gold"] = "Best tier" }, 0, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3282) });

            migrationBuilder.InsertData(
                table: "Investments",
                columns: new[] { "InvestorId", "ItemId", "Amount", "Tier", "UpdatedAt" },
                values: new object[,]
                {
                    { new Guid("13b8beec-ab9e-438c-8b44-1a4fbe7f357d"), new Guid("096544e2-3f7a-4c4b-a0f3-3a09d16942d4"), 40m, 1, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3402) },
                    { new Guid("13b8beec-ab9e-438c-8b44-1a4fbe7f357d"), new Guid("2a6e8599-aace-4515-84cd-2f721aa6bdb4"), 20m, 1, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3382) },
                    { new Guid("13b8beec-ab9e-438c-8b44-1a4fbe7f357d"), new Guid("db89cae7-a1e6-4791-bc0c-be04e3644acb"), 300m, 2, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3398) },
                    { new Guid("317f8bcd-f39e-443d-bc03-b12ae4b4a799"), new Guid("2a6e8599-aace-4515-84cd-2f721aa6bdb4"), 30m, 1, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3394) },
                    { new Guid("317f8bcd-f39e-443d-bc03-b12ae4b4a799"), new Guid("59e3af8b-8698-47c0-b839-393add3b7646"), 10000m, 3, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3381) },
                    { new Guid("317f8bcd-f39e-443d-bc03-b12ae4b4a799"), new Guid("d33ea736-e614-46e2-aabd-6549ef225570"), 100m, 2, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3379) },
                    { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Guid("096544e2-3f7a-4c4b-a0f3-3a09d16942d4"), 10m, 1, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3375) },
                    { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Guid("c4497c6a-7833-4021-8e53-0d8f5092ebd6"), 20000m, 3, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3393) },
                    { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Guid("db89cae7-a1e6-4791-bc0c-be04e3644acb"), 200m, 2, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3384) },
                    { new Guid("4c34f79f-c3a7-4dfa-9119-c320062f5683"), new Guid("f73c580c-5d6e-4b9e-8203-1ec287da619f"), 30000m, 3, new DateTime(2023, 5, 18, 11, 49, 23, 516, DateTimeKind.Utc).AddTicks(3400) }
                });

            migrationBuilder.CreateIndex(
                name: "IX_Users_CountryId",
                table: "Users",
                column: "CountryId");

            migrationBuilder.CreateIndex(
                name: "IX_Items_CountryId",
                table: "Items",
                column: "CountryId");

            migrationBuilder.AddForeignKey(
                name: "FK_Items_Countries_CountryId",
                table: "Items",
                column: "CountryId",
                principalTable: "Countries",
                principalColumn: "Id",
                onDelete: ReferentialAction.Cascade);

            migrationBuilder.AddForeignKey(
                name: "FK_Users_Countries_CountryId",
                table: "Users",
                column: "CountryId",
                principalTable: "Countries",
                principalColumn: "Id",
                onDelete: ReferentialAction.Cascade);
        }
    }
}
